...
class SignupScreen : BaseFragment<SignupScreenViewModel, ScreenSignupBinding>() {
    ...
    private fun checkFormValidate() : Boolean {
        ...
        if (viewBind.edtPwd.text.length < 6) {
            showError(getString(R.string.error), getString(R.string.password_must_contain_at_least_6_character))
            return false
        }
        if (!TextUtils.equals(viewBind.edtPwd.text, viewBind.edtConfirmPwd.text)){
            showError(getString(R.string.passwords_not_matched), "")
            return false
        }
        return true
    }
